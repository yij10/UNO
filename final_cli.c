...
char    my_id[id_lenth+1];
char    token[id_lenth+1];     //不包括換行
char    *ptr;
...
void login () {
    bool valid = true;
    char id[id_lenth+1];
    printf("登入階段, 請輸入玩家名稱(限大小寫字母, 限長%d。\n", id_lenth);
...
void goto_room(bool random) {
    char    player[9][id_lenth+1];
    char    *tt = NULL;
    char    room_id[id_lenth+1], master[id_lenth+1];
    int     maxfd = 0;
    fd_set  rset;


    for(int i=0; i<9; i++){
        for(int j=0; j<id_lenth+1; j++){
            player[i][j] = '\0';
        }
    }
    printf("\033[2J\033[1;1H");
...
void draw_player(int row, int col, int order, char player[], int remain, bool uno, char whos_turn[]){
    if (strcmp(player, whos_turn)==0) printf("\033[31m\033[%d;%dH", row, col);
    else printf("\033[35m\033[%d;%dH", row, col);
...
void draw_table (int n, char player[][id_lenth+1], int player_remain[], bool uno[], char broadcast[], char hint[], bool direct, char whos_turn[], 
char my_card_ori[], char new_card_ori[], int card_remain, int table_card_num, char table_card[], int time, bool new_message) {
    char    my_card[MAXLINE], new_card[MAXLINE];
    strcpy(my_card, my_card_ori);
    strcpy(new_card, new_card_ori);

    bool my_turn = !strcmp(my_id, whos_turn);
    ...
    printf("\033[36m\033[1;1H廣播:     %s\033[0m", broadcast);

    if(direct==0) printf("出牌方向: 順時針(...-> 1 -> 2 -> 3 ->...)");
    else printf("出牌方向: 逆時針(...<- 1 <- 2 <- 3...)");

    if (my_turn) printf("\033[2;52H 輪到 player %d (%s)                      \033[31m輪到您了!\033[0m\n", order_idx+1, whos_turn);
    ...
    //draw player
    if(n==2) {
        draw_player(17, 58, my_order, player[my_order-1], player_remain[my_order-1], uno[my_order-1], whos_turn);
        draw_player(4, 58, (my_order+1)%2==0 ? my_order+1 : (my_order+1)%2, player[(my_order)%2], player_remain[(my_order)%2], uno[(my_order)%2], whos_turn);
    } else if(n==3) {
        draw_player(17, 58, my_order, player[my_order-1], player_remain[my_order-1], uno[my_order-1], whos_turn);
        draw_player(4, 1, (my_order+1)%3==0 ? my_order+1 : (my_order+1)%3, player[(my_order)%3], player_remain[(my_order)%3], uno[(my_order)%3], whos_turn);
        draw_player(4, 116, (my_order+2)%3==0 ? my_order+2 : (my_order+2)%3, player[(my_order+1)%3], player_remain[(my_order+1)%3], uno[(my_order+1)%3], whos_turn);
    } else if(n==4) {
        draw_player(17, 58, my_order, player[my_order-1], player_remain[my_order-1], uno[my_order-1], whos_turn);
        draw_player(10, 1, (my_order+1)%4==0 ? my_order+1 : (my_order+1)%4, player[(my_order)%4], player_remain[(my_order)%4], uno[(my_order)%4], whos_turn);
        draw_player(4, 58, (my_order+2)%4==0 ? my_order+2 : (my_order+2)%4, player[(my_order+1)%4], player_remain[(my_order+1)%4], uno[(my_order+1)%4], whos_turn);
        draw_player(10, 116, (my_order+3)%4==0 ? my_order+3 : (my_order+3)%4, player[(my_order+2)%4], player_remain[(my_order+2)%4], uno[(my_order+2)%4], whos_turn);
    } else if (n == 5) {
        draw_player(17, 58, my_order, player[my_order - 1], player_remain[my_order - 1], uno[my_order - 1], whos_turn);
        draw_player(13, 1, (my_order + 1) % 5 == 0 ? my_order + 1 : (my_order + 1) % 5, player[(my_order) % 5], player_remain[(my_order) % 5], uno[(my_order) % 5], whos_turn);
        draw_player(4, 20, (my_order + 2) % 5 == 0 ? my_order + 2 : (my_order + 2) % 5, player[(my_order + 1) % 5], player_remain[(my_order + 1) % 5], uno[(my_order + 1) % 5], whos_turn);
        draw_player(4, 96, (my_order + 3) % 5 == 0 ? my_order + 3 : (my_order + 3) % 5, player[(my_order + 2) % 5], player_remain[(my_order + 2) % 5], uno[(my_order + 2) % 5], whos_turn);
        draw_player(13, 116, (my_order + 4) % 5 == 0 ? my_order + 4 : (my_order + 4) % 5, player[(my_order + 3) % 5], player_remain[(my_order + 3) % 5], uno[(my_order + 3) % 5], whos_turn);
    } else if (n == 6) {
        draw_player(17, 58, my_order, player[my_order - 1], player_remain[my_order - 1], uno[my_order - 1], whos_turn);
        draw_player(16, 15, (my_order + 1) % 6 == 0 ? my_order + 1 : (my_order + 1) % 6, player[(my_order) % 6], player_remain[(my_order) % 6], uno[(my_order) % 6], whos_turn);
        draw_player(6, 1, (my_order + 2) % 6 == 0 ? my_order + 2 : (my_order + 2) % 6, player[(my_order + 1) % 6], player_remain[(my_order + 1) % 6], uno[(my_order + 1) % 6], whos_turn);
        draw_player(4, 58, (my_order + 3) % 6 == 0 ? my_order + 3 : (my_order + 3) % 6, player[(my_order + 2) % 6], player_remain[(my_order + 2) % 6], uno[(my_order + 2) % 6], whos_turn);
        draw_player(6, 116, (my_order + 4) % 6 == 0 ? my_order + 4 : (my_order + 4) % 6, player[(my_order + 3) % 6], player_remain[(my_order + 3) % 6], uno[(my_order + 3) % 6], whos_turn);
        draw_player(16, 105, (my_order + 5) % 6 == 0 ? my_order + 5 : (my_order + 5) % 6, player[(my_order + 4) % 6], player_remain[(my_order + 4) % 6], uno[(my_order + 4) % 6], whos_turn);
    } else if (n == 7) {
        draw_player(17, 58, my_order, player[my_order - 1], player_remain[my_order - 1], uno[my_order - 1], whos_turn);
        draw_player(16, 10, (my_order + 1) % 7 == 0 ? my_order + 1 : (my_order + 1) % 7, player[(my_order) % 7], player_remain[(my_order) % 7], uno[(my_order) % 7], whos_turn);
        draw_player(7, 1, (my_order + 2) % 7 == 0 ? my_order + 2 : (my_order + 2) % 7, player[(my_order + 1) % 7], player_remain[(my_order + 1) % 7], uno[(my_order + 1) % 7], whos_turn);
        draw_player(4, 32, (my_order + 3) % 7 == 0 ? my_order + 3 : (my_order + 3) % 7, player[(my_order + 2) % 7], player_remain[(my_order + 2) % 7], uno[(my_order + 2) % 7], whos_turn);
        draw_player(4, 85, (my_order + 4) % 7 == 0 ? my_order + 4 : (my_order + 4) % 7, player[(my_order + 3) % 7], player_remain[(my_order + 3) % 7], uno[(my_order + 3) % 7], whos_turn);
        draw_player(7, 116, (my_order + 5) % 7 == 0 ? my_order + 5 : (my_order + 5) % 7, player[(my_order + 4) % 7], player_remain[(my_order + 4) % 7], uno[(my_order + 4) % 7], whos_turn);
        draw_player(16, 110, (my_order + 6) % 7 == 0 ? my_order + 6 : (my_order + 6) % 7, player[(my_order + 5) % 7], player_remain[(my_order + 5) % 7], uno[(my_order + 5) % 7], whos_turn);
    } else if (n == 8) {
        draw_player(17, 58, my_order, player[my_order - 1], player_remain[my_order - 1], uno[my_order - 1], whos_turn);
        draw_player(17, 20, (my_order + 1) % 8 == 0 ? my_order + 1 : (my_order + 1) % 8, player[(my_order) % 8], player_remain[(my_order) % 8], uno[(my_order) % 8], whos_turn);
        draw_player(11, 1, (my_order + 2) % 8 == 0 ? my_order + 2 : (my_order + 2) % 8, player[(my_order + 1) % 8], player_remain[(my_order + 1) % 8], uno[(my_order + 1) % 8], whos_turn);
        draw_player(4, 20, (my_order + 3) % 8 == 0 ? my_order + 3 : (my_order + 3) % 8, player[(my_order + 2) % 8], player_remain[(my_order + 2) % 8], uno[(my_order + 2) % 8], whos_turn);
        draw_player(4, 58, (my_order + 4) % 8 == 0 ? my_order + 4 : (my_order + 4) % 8, player[(my_order + 3) % 8], player_remain[(my_order + 3) % 8], uno[(my_order + 3) % 8], whos_turn);
        draw_player(4, 96, (my_order + 5) % 8 == 0 ? my_order + 5 : (my_order + 5) % 8, player[(my_order + 4) % 8], player_remain[(my_order + 4) % 8], uno[(my_order + 4) % 8], whos_turn);
        draw_player(11, 116, (my_order + 6) % 8 == 0 ? my_order + 6 : (my_order + 6) % 8, player[(my_order + 5) % 8], player_remain[(my_order + 5) % 8], uno[(my_order + 5) % 8], whos_turn);
        draw_player(17, 96, (my_order + 7) % 8 == 0 ? my_order + 7 : (my_order + 7) % 8, player[(my_order + 6) % 8], player_remain[(my_order + 6) % 8], uno[(my_order + 6) % 8], whos_turn);
    } else if (n == 9) {
        draw_player(17, 58, my_order, player[my_order - 1], player_remain[my_order - 1], uno[my_order - 1], whos_turn);
        draw_player(17, 30, (my_order + 1) % 9 == 0 ? my_order + 1 : (my_order + 1) % 9, player[(my_order) % 9], player_remain[(my_order) % 9], uno[(my_order) % 9], whos_turn);
        draw_player(13, 1, (my_order + 2) % 9 == 0 ? my_order + 2 : (my_order + 2) % 9, player[(my_order + 1) % 9], player_remain[(my_order + 1) % 9], uno[(my_order + 1) % 9], whos_turn);
        draw_player(5, 10, (my_order + 3) % 9 == 0 ? my_order + 3 : (my_order + 3) % 9, player[(my_order + 2) % 9], player_remain[(my_order + 2) % 9], uno[(my_order + 2) % 9], whos_turn);
        draw_player(4, 40, (my_order + 4) % 9 == 0 ? my_order + 4 : (my_order + 4) % 9, player[(my_order + 3) % 9], player_remain[(my_order + 3) % 9], uno[(my_order + 3) % 9], whos_turn);
        draw_player(4, 78, (my_order + 5) % 9 == 0 ? my_order + 5 : (my_order + 5) % 9, player[(my_order + 4) % 9], player_remain[(my_order + 4) % 9], uno[(my_order + 4) % 9], whos_turn);
        draw_player(5, 110, (my_order + 6) % 9 == 0 ? my_order + 6 : (my_order + 6) % 9, player[(my_order + 5) % 9], player_remain[(my_order + 5) % 9], uno[(my_order + 5) % 9], whos_turn);
        draw_player(13, 116, (my_order + 7) % 9 == 0 ? my_order + 7 : (my_order + 7) % 9, player[(my_order + 6) % 9], player_remain[(my_order + 6) % 9], uno[(my_order + 6) % 9], whos_turn);
        draw_player(17, 88, (my_order + 8) % 9 == 0 ? my_order + 8 : (my_order + 8) % 9, player[(my_order + 7) % 9], player_remain[(my_order + 7) % 9], uno[(my_order + 7) % 9], whos_turn);
    } else if (n == 10) {
        draw_player(17, 58, my_order, player[my_order - 1], player_remain[my_order - 1], uno[my_order - 1], whos_turn);
        draw_player(17, 30, (my_order + 1) % 10 == 0 ? my_order + 1 : (my_order + 1) % 10, player[(my_order) % 10], player_remain[(my_order) % 10], uno[(my_order) % 10], whos_turn);
        draw_player(15, 1, (my_order + 2) % 10 == 0 ? my_order + 2 : (my_order + 2) % 10, player[(my_order + 1) % 10], player_remain[(my_order + 1) % 10], uno[(my_order + 1) % 10], whos_turn);
        draw_player(6, 1, (my_order + 3) % 10 == 0 ? my_order + 3 : (my_order + 3) % 10, player[(my_order + 2) % 10], player_remain[(my_order + 2) % 10], uno[(my_order + 2) % 10], whos_turn);
        draw_player(4, 30, (my_order + 4) % 10 == 0 ? my_order + 4 : (my_order + 4) % 10, player[(my_order + 3) % 10], player_remain[(my_order + 3) % 10], uno[(my_order + 3) % 10], whos_turn);
        draw_player(4, 58, (my_order + 5) % 10 == 0 ? my_order + 5 : (my_order + 5) % 10, player[(my_order + 4) % 10], player_remain[(my_order + 4) % 10], uno[(my_order + 4) % 10], whos_turn);
        draw_player(4, 86, (my_order + 6) % 10 == 0 ? my_order + 6 : (my_order + 6) % 10, player[(my_order + 5) % 10], player_remain[(my_order + 5) % 10], uno[(my_order + 5) % 10], whos_turn);
        draw_player(6, 116, (my_order + 7) % 10 == 0 ? my_order + 7 : (my_order + 7) % 10, player[(my_order + 6) % 10], player_remain[(my_order + 6) % 10], uno[(my_order + 6) % 10], whos_turn);
        draw_player(15, 116, (my_order + 8) % 10 == 0 ? my_order + 8 : (my_order + 8) % 10, player[(my_order + 7) % 10], player_remain[(my_order + 7) % 10], uno[(my_order + 7) % 10], whos_turn);
        draw_player(17, 86, (my_order + 9) % 10 == 0 ? my_order + 9 : (my_order + 9) % 10, player[(my_order + 8) % 10], player_remain[(my_order + 8) % 10], uno[(my_order + 8) % 10], whos_turn);
    }
    ...
    // if (new_message) printf("\033[31m\033[30;111H您有新訊息喔\n\033[0m");
    if (new_message) printf("\033[31;111H輸入chat進入聊天室\033[31m(new)\033[0m\n");
    else printf("\033[31;111H輸入chat進入聊天室\n");
...
void game() {
    ...
    int     n = 0;
    int     n_remain = 0;
    char    player[10][id_lenth+1];
    int     player_remain[10] = {0};
    bool    uno[10] = {false};
    char    broadcast[MAXLINE] = "~ ~ ~ Hello ~ ~ ~\n";
    char    hint[MAXLINE] = "";
    bool    direct = 0;
    char    whos_turn[id_lenth+1];
        ...
        if (FD_ISSET(sockfd, &rset)) {                              //收到server更新遊戲狀態
            ...
            while(recvline[cut_count]!='\0') {
                ...
                if (line[0]=='1' && line[1]=='0') {         //回合狀態
                    ...
                    dont_draw = true;
                    strcpy(record[record_index % message_num], &line[3]);
                    record_index++;

                    if (new_message == false && !chat) {
                        printf("\033[s");
                        printf("\033[31;111H輸入chat進入聊天室\033[31m(new)\033[0m\n");
                        printf("\033[u");
                        fflush(stdout);
                    } 
                ...
                if (chat) {                             //畫出聊天室
                    tem = system("clear");                         
                    printf("hi, 這裡是聊天室! 請勿一次輸入超過100字         輸入chat進入退出聊天室\n");
                    for(int i=0; i<message_num; i++) {
                    ...
                    if (!dont_draw) {
                        printf("\033[s");
                        draw_table(n, player, player_remain, uno, broadcast, hint, direct, whos_turn, my_card, new_card, card_remain, table_card_num, table_card, remaining_time, new_message);
